"""
Nexus — WebSocket Connection Manager with Redis Backing

Responsibilities:
  • Track active WebSocket connections in-memory (per-process)
  • Track online presence in Redis (cross-process / cross-node)
  • Deliver messages to online recipients instantly
  • Queue messages in Redis for offline recipients
...
class ConnectionManager:
    ...
    Usage:
      ...
      await manager.initialize()          # call once at app startup
      await manager.connect(user_id, ws)
      await manager.send_to_user(target_id, payload_dict)
      await manager.disconnect(user_id)
      await manager.shutdown()            # call once at app shutdown
    """
    ...
    async def send_to_user(self, user_id: str, payload: dict) -> bool:
        ...
        • If the user has a local WebSocket → send directly.
        • If the user is online elsewhere   → queue.
        • If the user is offline            → queue for later delivery.

        Returns True if delivered in this process, False if queued.
        """
    ...
    async def _flush_offline_queue(
        ...
        while True:
            ...
            try:
                ...
                # If sending fails, push it back so it's not lost
                if self._use_redis and self._redis:
                    try:
